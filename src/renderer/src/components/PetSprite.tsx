...
import type { PetDefinition, PetState, SpriteSheetSpec } from '@shared/types'
...
/**
 * v0.4：PetDex spritesheet 宠物的逐帧动画渲染（Canvas）。
 * 按当前 PetState 映射到精灵图的某一行，循环播放该行的帧；换 state 立即切行。
 * 尊重 prefers-reduced-motion：减少动效时只画该状态第一帧、不循环。
 */
function SpriteSheetPet({ sprite, state }: { sprite: SpriteSheetSpec; state: PetState }): JSX.Element {
  ...
  useEffect(() => {
    ...
    const anim = sprite.states[state] ?? sprite.states.idle
    const row = anim?.row ?? 0
    const frames = anim?.frames ?? 1
    const durationMs = anim?.durationMs ?? 150
    const { frameWidth: fw, frameHeight: fh, columns } = sprite
    ...
    const drawFrame = (i: number): void => {
      ...
      ctx.imageSmoothingEnabled = false // 像素图保持利落
      ctx.drawImage(img, col * fw, row * fh, fw, fh, 0, 0, fw, fh)
    }

    const tick = (t: number): void => {
      if (!last) last = t
      if (t - last >= durationMs) {
        last = t
    ...
    // 图片可能还没加载完，等它 onload 后画首帧
    const img = imgRef.current
    const start = (): void => {
      drawFrame(0)
      if (!reduce && frames > 1) raf = requestAnimationFrame(tick)
    }
    if (img && img.complete && img.naturalWidth > 0) start()
    else if (img) img.onload = start
...
export default function PetSprite({ pet, state }: PetSpriteProps): JSX.Element {
  // v0.4：PetDex spritesheet 宠物，逐帧动画渲染
  if (pet.kind === 'spritesheet' && pet.sprite) {
    return <SpriteSheetPet sprite={pet.sprite} state={state} />
  }
